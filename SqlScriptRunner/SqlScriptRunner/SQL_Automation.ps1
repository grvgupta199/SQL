...
Invoke-Sqlcmd -ServerInstance dwettdcg23413.com -Database 'Master' -Query 'SELECT GETDATE()' -Verbose
...
# Write-Host 'Running Script : ' $s.FullName -BackgroundColor Yellow -ForegroundColor black
   ...
   Write-Host $LineSeprater
   #Write-Host $LineSeprater
    $runningDirScript=$s.Directory.FullName
   $dirName=$null
   $id=$null
   $currentId=$null
